...
def learn(env,
    ...
    on_policy_qvalues = tf.gather_nd(qvalues, indices)

    max_qvalues = tf.reduce_max(qvalues, axis=1)

    total_error = tf.reduce_sum(tf.square(rew_t_ph - on_policy_qvalues))
    ...
    # create refresh function, used by the replay buffer
    # change this to switch between Peng's and Watkin's Q-Learning
    refresh_func = lambda x: session.run(max_qvalues, feed_dict={obs_t_ph: x})

    # construct the replay buffer
    replay_buffer = ReplayBuffer(replay_buffer_size, frame_history_len, gamma, Lambda, refresh_func)

    # for benchmarking
    bm_replay_buffer = ReplayBuffer(replay_buffer_size, frame_history_len, gamma, Lambda, refresh_func)
    ...
    def benchmark(n_episodes):
        for i in range(n_episodes):
            ...
            while not done:
                bm_replay_buffer.store_frame(obs)
                ...
                bm_replay_buffer.store_effect(action, reward, done)
    ...
    for t in itertools.count():
        ...
        if t % target_update_freq == 0:
            ...
            print('Refresh took', time.time() - time_before_refresh, 'seconds', flush=True)
