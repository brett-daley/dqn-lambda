...
class TestCaseLegacyReplayMemory(TestCaseCore):
    ...
    def test_1step(self):
        ...
        self.fill(m)
        states, actions, returns = m._sample(np.arange(0, 9))

        self.assertNumpyEqual(states,  [    0,     1,    2,     3,     4,    5,     6,     7,     8])
    ...
    def test_nstep(self):
        ...
        self.fill(m)
        states, actions, returns = m._sample(np.arange(0, 7))

        self.assertNumpyEqual(states,  [   0,   1,    2,    3,     4,     5,   6])
