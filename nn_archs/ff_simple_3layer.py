...
class ff_simple_3layer(Network):
	...
	def create_Q_network(self):
		with tf.variable_scope(self.scope):
			...
			fc2 = tf.nn.relu(tf.matmul(fc1, self.var['W_l2']) + self.var['b_l2'])

			# hidden layer 2 - fc
			l3_n_hidden = self.n_actions
			self.var['W_l3'] = self.weight_variable(shape=[l2_n_hidden, l3_n_hidden])
			self.var['b_l3'] = self.bias_variable([l3_n_hidden])
			self.QValue = tf.matmul(fc2, self.var['W_l3']) + self.var['b_l3'] # This is QValue for ALL the possible actions
